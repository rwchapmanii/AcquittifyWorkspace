...
resource "aws_cloudwatch_log_group" "caselaw_ingest" {
  name              = "/ecs/${local.stack_name}/caselaw-ingest"
  retention_in_days = var.log_retention_days
...
locals {
  ...
  api_environment = [
    ...
    { name = "AUTH_EMAIL_SENDER", value = var.auth_email_sender },
    { name = "AUTH_EMAIL_REGION", value = var.auth_email_region != "" ? var.auth_email_region : var.aws_region },
    { name = "AUTH_MFA_ISSUER", value = var.auth_mfa_issuer },
  ...
  caselaw_job_secrets = concat(
    local.worker_secrets,
    var.courtlistener_api_token_secret_arn != "" ? [{ name = "COURTLISTENER_API_TOKEN", valueFrom = var.courtlistener_api_token_secret_arn }] : []
  )

  caselaw_job_environment = concat(
    local.worker_environment,
    [
      { name = "ACQ_CASELAW_TIMEZONE", value = var.caselaw_schedule_timezone },
      { name = "ACQ_CASELAW_RUNTIME_HOURS", value = tostring(var.caselaw_job_max_runtime_hours) },
      { name = "ACQ_CASELAW_BACKFILL_START_DATE", value = var.caselaw_job_backfill_start_date },
      { name = "ACQ_CASELAW_MAX_COURT_DATE_QUERIES", value = tostring(var.caselaw_job_max_court_date_queries) },
      { name = "ACQ_CASELAW_PAGE_SIZE", value = tostring(var.caselaw_job_page_size) },
      { name = "ACQ_CASELAW_MAX_PAGES_PER_QUERY", value = tostring(var.caselaw_job_max_pages_per_query) },
      { name = "ACQ_CASELAW_REQUEST_TIMEOUT_SECONDS", value = tostring(var.caselaw_job_request_timeout_seconds) },
      { name = "ACQ_CASELAW_REQUEST_PAUSE_SECONDS", value = tostring(var.caselaw_job_request_pause_seconds) },
      { name = "ACQ_CASELAW_REQUEST_RETRIES", value = tostring(var.caselaw_job_request_retries) },
      { name = "ACQ_CASELAW_ONLY_COURTS", value = var.caselaw_job_only_courts },
      { name = "ACQ_CASELAW_STATE_KEY", value = "courtlistener_federal_criminal_scheduler" },
      { name = "ACQ_CASELAW_LOG_PATH", value = "/tmp/caselaw_nightly_ingest.jsonl" },
    ]
  )
...
resource "aws_ecs_task_definition" "caselaw_ingest" {
  family                   = "${local.stack_name}-caselaw-ingest"
  requires_compatibilities = ["FARGATE"]
  network_mode             = "awsvpc"
  cpu                      = tostring(var.caselaw_task_cpu)
  memory                   = tostring(var.caselaw_task_memory)
  execution_role_arn       = aws_iam_role.ecs_task_execution.arn
  task_role_arn            = aws_iam_role.ecs_task.arn
  ...
  container_definitions = jsonencode([
    {
      ...
      image     = local.api_image
      essential = true
      command = var.caselaw_job_include_quasi_criminal ? [
        "python",
        "scripts/caselaw_nightly_ingest_job.py",
        "--max-runtime-hours",
        tostring(var.caselaw_job_max_runtime_hours),
        ] : [
        "python",
        "scripts/caselaw_nightly_ingest_job.py",
        "--max-runtime-hours",
      ...
      environment = local.caselaw_job_environment
      secrets     = local.caselaw_job_secrets
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          awslogs-group         = aws_cloudwatch_log_group.caselaw_ingest.name
          awslogs-region        = var.aws_region
          awslogs-stream-prefix = "caselaw-ingest"
        }
      }
    }
  ])
