...
def run_pass4(*, session: Session, document_id: str) -> PassRun:
    ...
    prompt = build_pass4_prompt(text)
    client = LLMClient()
    settings = get_settings()
    raw_text = client.complete_text(
        prompt=prompt, model=settings.llm_model, temperature=0.1
    )

    schema = Pass4Schema.model_json_schema()
    ...
    if not response_json or (validation and not validation.ok):
        ...
        prompt_id = PASS4_REPAIR_PROMPT_ID
        prompt_hash = client.prompt_hash(
            build_pass4_repair_prompt(raw_text, json.dumps(schema))
        )
    ...
    parsed = Pass4Schema.model_validate(response_json)

    _clear_latest(session, document_id=document_id, pass_num=4)
